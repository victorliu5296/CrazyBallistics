...
public partial struct PolynomialDouble
{
    ...
    public PolynomialDouble Normalized()
    {
        ...
        double scalingFactor = Coefficients[^1];
        var normalizedCoefficients = Coefficients.Select(c => c / scalingFactor).ToArray();
        return new PolynomialDouble(normalizedCoefficients);
    }
    ...
    public PolynomialDouble DerivativeTimesX()
    {
        var weightedDerivativeCoeffs = new double[Coefficients.Length];
        for (int i = 0; i < Coefficients.Length; i++)
        {
            weightedDerivativeCoeffs[i] = Coefficients[i] * i;
        }
        return new PolynomialDouble(weightedDerivativeCoeffs);
    }
    ...
    public PolynomialDouble PolynomialDerivative()
    {
        if (Coefficients.Length <= 1) return new PolynomialDouble([0]);

        var derivativeCoeffs = new double[Coefficients.Length - 1];
        for (int i = 1; i < Coefficients.Length; i++)
        {
            derivativeCoeffs[i - 1] = Coefficients[i] * i;
        }
        return new PolynomialDouble(derivativeCoeffs);
    }
    ...
    /// </summary>
    /// <returns>A new <see cref="PolynomialDouble"/> instance with coefficients shifted by one degree lower.</returns>
    public PolynomialDouble ShiftCoefficientsBy1()
    {
        ...
        double[] shiftedCoefficients = new double[Coefficients.Length - 1];
        ...
        return new PolynomialDouble(shiftedCoefficients);
    }
    ...
    public PolynomialDouble MakeSquarefree()
    {
        ...
        if (gcd.Coefficients.Length == 1 && Math.Abs(gcd.Coefficients[0] - 1) < double.Epsilon)
        {
            return this;
        }

        //if (Math.Sign(this.Coefficients.Last()) != Math.Sign(gcd.Coefficients.Last()))
        //{
        //    for (int i = 0; i < Coefficients.Length; i++)
        //    {
    ...
    public static (PolynomialDouble Quotient, PolynomialDouble Remainder) PolynomialDivision(PolynomialDouble dividend, PolynomialDouble divisor)
    {
        var dividendCoeffs = dividend.Coefficients;
        var divisorCoeffs = divisor.Coefficients;

        if (divisorCoeffs.All(coefficient => Math.Abs(coefficient) < double.Epsilon))
        {
        ...
        if (len_diff < 0)
        {
            ...
            return (new PolynomialDouble([0]), dividend);
        }

        var quotientCoeffs = new List<double>();
        var remainderCoeffs = new List<double>(dividendCoeffs);
        double normalizer = divisorCoeffs.Last();

        for (int i = 0; i <= len_diff; i++)
        {
            // Calculate the scale factor for the divisor to subtract from the dividend
            double scale = remainderCoeffs[^1] / normalizer;
        ...
        while (remainderCoeffs.Count > 1 && Math.Abs(remainderCoeffs.Last()) < double.Epsilon)
        {
            remainderCoeffs.RemoveAt(remainderCoeffs.Count - 1);
        }
        ...
        return (new PolynomialDouble([.. quotientCoeffs]), new PolynomialDouble([.. remainderCoeffs]));
    }
    ...
    /// </remarks>
    public static PolynomialDouble PolynomialGCD(PolynomialDouble a, PolynomialDouble b)
    {
        // Continuously apply the Euclidean algorithm until a remainder of zero is found.
        while (b.Coefficients.Any(c => Math.Abs(c) > double.Epsilon))
        {
